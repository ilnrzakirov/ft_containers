...
namespace ft {
    ...
    public:
        ...
        RanIt base() const {
            return (current); }
        Rt operator*() const {
            RanIt tmp = current; return (*--tmp); }
        Pt operator->() const {
            return (&**this); }
        Myt &operator++() {
            --current; return (*this); }
        Myt operator++(int) {
            Myt Tmp = *this; --current; return (Tmp); }
        Myt &operator--() {
            ++current; return (*this); }
        Myt operator--(int) {
            Myt Tmp = *this; ++current; return (Tmp); }
    ...
    typename reverse_iterator<RanIt>::D operator - (const reverse_iterator<RanIt>& X, const reverse_iterator<RanIt>& Y) {
